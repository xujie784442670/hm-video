...
const json=ref()
onBeforeMount(()=>{
  request.get("advertise").then(rs=>{
      json.value=rs.data.data
...
export default{
  ...
  methods:{
    ...
    handleScroll (event) {
      console.log("滚动条高度为：",this.scrollTop)
    ...
    init(json){
      ...
      obj.url = json.url;
      obj.posterUrl=json.posterUrl;
      console.log("json.posterUrl:"+json.posterUrl)
      console.log("json.url:"+json.posterUrl)
    ...
    float(index) {
      ...
      obj.style.left = obj.left + "px";
      obj.style.top = obj.top + "px";*/
      obj.style.top=obj.top+this.scrollTop+"px";
    }
